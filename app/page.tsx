...
export default function Page() {
  ...
  async function loadAnalytics(from?:string,to?:string):Promise<Analytics|null>{
    ...
    const result:Analytics={total,cash,card,transfer,mixed,discounts,checks:checkRows.length,guests,avg:checkRows.length?total/checkRows.length:0,avgGuest:guests?total/guests:0,top};
    setAnalytics(result);
    return result;
  }
  ...
  async function editOrderItemQty(order:Order,item:OrderItem,delta:number){
    if(profile?.role!=='waiter'&&profile?.role!=='bar'&&profile?.role!=='admin')return setToast('Редактировать заказ может только официант, бар или администратор');
  ...
  if(profile.role==='bar') return <main className="bar-tablet-shell">
    <BarTabletView profile={profile} tables={tables} orders={orders} summary={tableSummary} loading={loading} ordersLoading={ordersLoading} notificationsOn={notificationsOn}
      ...
      onNewOrder={(t)=>{setSelectedTable(t);setCart([]);setMenuCategory(null);setEditingBarOrderId(null);setBarDetail(null);setView('menu')}}
      onReadyAll={markOrderReady} onToggleItem={toggleItemReady} />
    ...
    {barDetail&&<BarDetailModal table={barDetail} orders={orders.filter(o=>o.table_id===barDetail.id&&o.status!=='done'&&o.status!=='cancelled')} canPay={true} onNew={()=>{setSelectedTable(barDetail);setCart([]);setMenuCategory(null);setEditingBarOrderId(null);setBarDetail(null);setView('menu')}} onAddToOrder={order=>{setSelectedTable(barDetail);setCart([]);setMenuCategory(null);setEditingBarOrderId(order.id);setBarDetail(null);setView('menu')}} onEditItem={editOrderItemQty} onPay={order=>{setBarDetail(null);openBarPayment(order)}} onDismiss={()=>setBarDetail(null)}/>}
...
function BarTabletView({profile,tables,orders,summary,loading,ordersLoading,notificationsOn,onRefresh,onEnableNotifications,onSignOut,onOpenTable,onNewOrder,onReadyAll,onToggleItem}:{profile:Profile,tables:RestaurantTable[],orders:Order[],summary:Record<number,{total:number;newCount:number;workCount:number;readyCount:number;orders:number}>,loading:boolean,ordersLoading:boolean,notificationsOn:boolean,onRefresh:()=>void,onEnableNotifications:()=>void,onSignOut:()=>Promise<void>,onOpenTable:(t:RestaurantTable)=>void,onNewOrder:(t:RestaurantTable)=>void,onReadyAll:(o:Order)=>Promise<void>,onToggleItem:(o:Order,i:OrderItem)=>Promise<void>}){
  ...
  return <div className="bar-tablet-app">
    ...
    <div className="bar-tablet-body">
      ...
      <main className="bar-tablet-main"><div className="bar-main-head"><div><h2>Неоплаченные столы</h2><p>{unpaid.length} занято · к доплате <b>{money(totalDue)}</b></p></div><button className="bar-refresh-main" onClick={onRefresh}><RefreshCw size={16}/> Обновить</button></div><div className="bar-unpaid-grid">{unpaid.length?unpaid.map(t=>{const s=summary[t.id]||{total:0,newCount:0,workCount:0,readyCount:0,orders:0};const due=Math.max(0,s.total-Number(t.deposit_total||0));return <button key={t.id} className="bar-unpaid-card" onClick={()=>onOpenTable(t)}><div className="bar-unpaid-top"><b>{t.name}</b><span>{due?money(due):'Оплачено'}</span></div><div className="bar-unpaid-meta">👥 {t.guests} · {s.orders} заказов</div><div className="bar-unpaid-bottom"><span className={s.newCount?'hot':''}>{s.newCount?'Новые '+s.newCount:'Нет новых'}</span><span>{s.workCount?'В работе '+s.workCount:''}</span><span>{s.readyCount?'Готово '+s.readyCount:''}</span></div></button>}) : <div className="bar-empty"><ReceiptText size={26}/><b>Все столы оплачены</b><span>Новых неоплаченных столов нет</span></div>}</div><div className="bar-bottom-strip"><button onClick={()=>onNewOrder(active.find(t=>t.is_bar)||active.find(t=>t.status==='free')||active[0])} disabled={!active.length}><Plus size={18}/> Новый чек</button><div><b>{orders.length}</b> открытых заказов <span>·</span> <b>{pendingOrders.length}</b> с неготовыми позициями</div></div></main>
      <aside className="bar-tablet-right"><section className="bar-queue"><div className="bar-queue-head"><h3>Новые заказы</h3><b>{newOrders.length}</b><span>🔊</span></div>{newOrders.length?newOrders.map(o=><button key={o.id} className="bar-order-row" onClick={()=>onOpenTable(tables.find(t=>t.id===o.table_id) || active[0])}><div><strong>{o.table_name}</strong><small>Заказ #{o.id} · {o.items.length} поз.</small></div><span>{new Date(o.created_at).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</span></button>):<div className="bar-queue-empty">Новых заказов нет</div>}</section><section className="bar-queue"><div className="bar-queue-head"><h3>Неготовые позиции</h3><b className="orange-count">{pendingOrders.reduce((n,o)=>n+o.items.filter(i=>i.item_status!=='ready'&&i.item_status!=='done').length,0)}</b></div>{pendingOrders.length?pendingOrders.map(o=><div key={o.id} className="bar-pending-order"><div className="bar-pending-title"><strong>{o.table_name} · #{o.id}</strong><button onClick={()=>onReadyAll(o)}>Готово всё</button></div>{o.items.filter(i=>i.item_status!=='ready'&&i.item_status!=='done').slice(0,4).map(i=><button key={i.id} className="bar-pending-item" onClick={()=>onToggleItem(o,i)}><span>{i.qty}× {i.item_name}</span><span>Не готово</span></button>)}</div>):<div className="bar-queue-empty">Все позиции готовы</div>}</section></aside>
    </div>
  </div>
}
...
function AdminMenu({categories,items,onReload,onClose,toast,receipts,receiptsLoading,onReloadReceipts,analytics,onReloadAnalytics,currentShift,onReloadShift,onAuditReload,onLog,auditLogs,auditLoading,roundingStep,onSaveRounding,auditRetentionHours,onSaveAuditRetention,tables,orders}:{categories:Category[],items:MenuItem[],onReload:()=>Promise<void>,onClose:()=>void,toast:(s:string)=>void,receipts:Receipt[],receiptsLoading:boolean,onReloadReceipts:()=>Promise<void>,analytics:Analytics,onReloadAnalytics:(from?:string,to?:string)=>Promise<Analytics|null>,currentShift:Shift|null,onReloadShift:()=>Promise<void>,onAuditReload:()=>Promise<void>,onLog:(action:string,entityType:string,entityId?:string|number|null,details?:Record<string,any>)=>Promise<void>,auditLogs:AuditLog[],auditLoading:boolean,roundingStep:number,onSaveRounding:(step:number)=>Promise<void>,auditRetentionHours:number,onSaveAuditRetention:(hours:number)=>Promise<void>,tables:RestaurantTable[],orders:Order[]}){
  ...
  const emptyAnalytics:Analytics={total:0,cash:0,card:0,transfer:0,mixed:0,discounts:0,checks:0,guests:0,avg:0,avgGuest:0,top:[]};
  ...
  async function loadOverviewAnalytics(){
    ...
    const {data:openTables}=await sb.from('restaurant_tables').select('guests').eq('active',true).eq('is_bar',false).eq('status','busy');
    const openTableGuests=(openTables||[]).reduce((a,t)=>a+Math.max(0,Number((t as any).guests||0)),0);
