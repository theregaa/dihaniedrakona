...
type RestaurantTable = { id: number; name: string; is_vip: boolean; status: 'free' | 'busy'; guests: number; active_order_id: number | null; active: boolean };
...
type Staff = { id:string; full_name:string; role:string; active:boolean; email?:string|null; created_at?:string };
...
const icons: Record<string,string> = { 'Кальяны':'💨','Кофе':'☕','Чай':'🍵','Матча':'🍵','Б/А напитки':'🥤','Авторские напитки':'🍹','Снэки':'🍟','Сэндвичи':'🥪' };
...
export default function Page() {
  ...
  useEffect(()=>{ sb.auth.getSession().then(async({data})=>{ if(!data.session?.user)return; const {data:staff}=await sb.from('staff_profiles').select('id,full_name,role,active').eq('id',data.session.user.id).single(); if(staff?.active)setProfile({id:staff.id,full_name:staff.full_name,role:staff.role==='bar'?'waiter':staff.role}); }); },[sb]);
  useEffect(()=>{
    if(!profile)return;
    let alive=true;
  ...
  async function loadTables(){
    ...
    const {data,error:e}=await sb.from('restaurant_tables').select('id,name,is_vip,status,guests,active_order_id,active').eq('active',true).order('is_vip').order('id');
  ...
  async function loadOrders(show=true){
    ...
    try{
      ...
      const {data:os,error:e}=await sb.from('orders').select('id,table_id,staff_id,status,created_at,shift_id').eq('shift_id',shift.id).order('created_at',{ascending:false}).limit(80);
      if(e)throw e;
      ...
      setOrders((os||[]).map((o:any)=>{
        ...
        return {id:o.id,table_id:o.table_id,table_name:tableMap.get(o.table_id)||'Стол',staff_name:staffMap.get(o.staff_id)||'Сотрудник',status:aggregateOrderStatus(all),created_at:o.created_at,items:visible}
  ...
  async function signIn(){setError('');if(!login.trim()||!pin.trim())return setError('Введите email и PIN-код');if(!/^\d{6}$/.test(pin))return setError('PIN-код должен состоять из 6 цифр');setBusy(true);const {data,error:e}=await sb.auth.signInWithPassword({email:login.trim(),password:pin});if(e||!data.user){setBusy(false);return setError('Неверный email или PIN-код')}const {data:staff}=await sb.from('staff_profiles').select('id,full_name,role,active').eq('id',data.user.id).single();setBusy(false);if(!staff?.active){await sb.auth.signOut();return setError('Профиль сотрудника не найден или отключён.')}setProfile({id:staff.id,full_name:staff.full_name,role:staff.role==='bar'?'waiter':staff.role});await sb.from('audit_logs').insert({staff_id:staff.id,action:'Вошёл в систему',entity_type:'auth',details:{method:'email+pin'}})}
  ...
  async function openTable(t:RestaurantTable){if(t.status==='busy'){setTableDetail(t);return}const raw=window.prompt(`Количество гостей — ${t.name}`,'2');const guests=Math.max(1,Number.parseInt(raw||'2',10)||2);const {data,error:e}=await sb.from('restaurant_tables').update({status:'busy',guests}).eq('id',t.id).select('id,name,is_vip,status,guests,active_order_id,active').single();if(e)return setToast(e.message);setSelectedTable(data as RestaurantTable);setCart([]);setMenuCategory(null);setView('menu');await logAction('Открыл стол','table',t.id,{table:t.name,guests});await loadTables()}
  ...
  async function sendOrder(){
    ...
    const {error:te}=await sb.from('restaurant_tables').update({active_order_id:o.id,status:'busy'}).eq('id',selectedTable.id);
  ...
  const normal=tables.filter(t=>!t.is_vip),vip=tables.filter(t=>t.is_vip),shown=filter==='normal'?normal:filter==='vip'?vip:tables;
...
function MenuView({categories,items,category,setCategory,cart,addItem,changeQty,selectedTable,onSend,busy}:{categories:Category[],items:MenuItem[],category:number|null,setCategory:(n:number|null)=>void,cart:CartItem[],addItem:(i:MenuItem,d?:Record<string,string>)=>void,changeQty:(i:number,d:number)=>void,selectedTable:RestaurantTable|null,onSend:()=>void,busy:boolean}){
  const [strength,setStrength]=useState('Средний'),[picked,setPicked]=useState<MenuItem|null>(null),[hookahComment,setHookahComment]=useState(''),[itemNote,setItemNote]=useState(''),[pickedSize,setPickedSize]=useState<'small'|'large'>('small');
  ...
  return <div>{selectedTable&&<div className="selected-table-card"><div><div className="brand">ТЕКУЩИЙ СТОЛ</div><b>{selectedTable.name}</b><span className="muted"> · {selectedTable.guests} {plural(selectedTable.guests,'гость','гостя','гостей')}</span></div><button className="back-btn small-back" onClick={()=>setCategory(null)}>Меню</button></div>}{!category?<><div className="menu-category-list">{topCategories.map(c=><button key={c.id} className="menu-category" onClick={()=>setCategory(c.id)}><span><span className="category-icon">{c.icon||icons[c.name]||'🍽️'}</span><b>{c.name}</b><small>{countItems(c.id)} поз.</small></span><ChevronRight/></button>)}</div>{!topCategories.length&&<div className="placeholder"><div>🍽️</div><h2>Меню пока пустое</h2><p>Добавьте категории и позиции в Supabase.</p></div>}</>:<><button className="back-btn" onClick={()=>setCategory(current?.parent_id??null)}><ArrowLeft size={17}/> {current?.parent_id?'Назад':'Все категории'}</button>{children.length>0?<div className="menu-category-list">{children.map(c=><button key={c.id} className="menu-category" onClick={()=>setCategory(c.id)}><span><span className="category-icon">{c.icon||'🍽️'}</span><b>{c.name}</b><small>{countItems(c.id)} поз.</small></span><ChevronRight/></button>)}</div>:null}{directItems.length>0&&<div className="items-list">{directItems.map(i=><button className="menu-item" key={i.id} onClick={()=>openItem(i)}><div className="item-icon">{icons[categoryName(categories,topCategoryId(categories,i.category_id))]||'🍽️'}</div><div className="item-main"><b>{i.name}</b><small>{i.description||'Добавить в заказ'}</small></div><strong>{i.large_price!=null?`${money(i.price)} / ${money(i.large_price)}`:money(i.price)}</strong><ChevronRight size={17}/></button>)}</div>}{!children.length&&!directItems.length&&<div className="placeholder compact"><div>🍽️</div><p>В этой категории пока нет позиций.</p></div>}</>}{cart.length>0&&<div className="cart-card"><div className="row"><b>Текущий заказ</b><strong>{money(total)}</strong></div>{cart.map((x,i)=><div className="cart-line" key={i}><div><b>{x.item_name}</b><small>{Object.values(x.details).filter(Boolean).join(' · ')}</small></div><div className="qty"><button onClick={()=>changeQty(i,-1)}><Minus size={15}/></button><b>{x.qty}</b><button onClick={()=>changeQty(i,1)}><Plus size={15}/></button></div></div>)}<button className="orange-btn" onClick={onSend} disabled={busy}><Send size={17}/> ОТПРАВИТЬ ЗАКАЗ · {money(total)}</button></div>}</div>
}
function OrdersView({orders,loading,onStatus,role,filter,setFilter}:{orders:Order[],loading:boolean,onStatus:(o:Order)=>void,role:string,filter:'all'|'new'|'work'|'ready',setFilter:(x:'all'|'new'|'work'|'ready')=>void}){
  ...
  const deptStatus=(o:Order)=>{const mine=o.items;if(!mine.length)return 'new';if(mine.every(x=>x.item_status==='ready'||x.item_status==='done'))return 'ready';if(mine.some(x=>x.item_status==='work'||x.item_status==='ready'))return 'work';return 'new'};
  ...
  return <div><div className="role-banner"><b>{title}</b><span>{role==='admin'?'Администратор видит и контролирует все заказы':'Сотрудник видит все заказы и может взять любые позиции в работу'}</span></div>
    ...
    {loading&&!orders.length?<div className="loading-card"><Loader2 className="spin"/> Загружаем заказы...</div>:visible.map(o=><div className="order-card" key={o.id}>
      ...
      <div className="row order-footer"><strong>{money(o.items.reduce((s,x)=>s+Number(x.unit_price)*x.qty,0))}</strong>{actionLabel(o)?<button className="orange-small" onClick={()=>onStatus(o)}>{actionLabel(o)}</button>:deptStatus(o)==='ready'?<span className="muted ready-label"><CheckCircle2 size={15}/> Всё готово</span>:<span className="muted">Ожидаем работу</span>}</div>
    </div>)}
...
function AdminMenu({categories,items,onReload,onClose,toast,receipts,receiptsLoading,onReloadReceipts,analytics,onReloadAnalytics,currentShift,onReloadShift,onAuditReload,onLog,auditLogs,auditLoading}:{categories:Category[],items:MenuItem[],onReload:()=>Promise<void>,onClose:()=>void,toast:(s:string)=>void,receipts:Receipt[],receiptsLoading:boolean,onReloadReceipts:()=>Promise<void>,analytics:Analytics,onReloadAnalytics:(from?:string,to?:string)=>Promise<void>,currentShift:Shift|null,onReloadShift:()=>Promise<void>,onAuditReload:()=>Promise<void>,onLog:(action:string,entityType:string,entityId?:string|number|null,details?:Record<string,any>)=>Promise<void>,auditLogs:AuditLog[],auditLoading:boolean}){
  ...
  const [newStaff,setNewStaff]=useState({full_name:'',email:'',pin:'',role:'waiter'}),[staffSaving,setStaffSaving]=useState(false);
  const [editingStaff,setEditingStaff]=useState<string|null>(null),[staffDraft,setStaffDraft]=useState({full_name:'',role:'waiter',pin:''});
  ...
  async function loadAdminTables(){setTablesLoading(true);const {data,error}=await sb.from('restaurant_tables').select('id,name,is_vip,status,guests,active_order_id,active').order('is_vip').order('id');setTablesLoading(false);if(error)return toast(error.message);setAdminTables((data||[]) as RestaurantTable[])}
  ...
  async function createStaff(){if(!newStaff.full_name.trim()||!newStaff.email.trim()||!/^[0-9]{6}$/.test(newStaff.pin))return toast('Имя, email и PIN из 6 цифр обязательны');setStaffSaving(true);const r=await apiStaff('POST',{...newStaff,full_name:newStaff.full_name.trim(),email:newStaff.email.trim().toLowerCase()});setStaffSaving(false);if(!r.ok)return toast(r.message||'Не удалось создать сотрудника');setNewStaff({full_name:'',email:'',pin:'',role:'waiter'});toast('Сотрудник создан');await onLog('Создал сотрудника','staff',undefined,{name:newStaff.full_name.trim(),email:newStaff.email.trim().toLowerCase(),role:newStaff.role});await loadStaff()}
  function beginStaff(s:Staff){setEditingStaff(s.id);setStaffDraft({full_name:s.full_name,role:s.role,pin:''})}
  async function saveStaff(id:string){if(!staffDraft.full_name.trim())return toast('Введите имя');if(staffDraft.pin && !/^[0-9]{6}$/.test(staffDraft.pin))return toast('PIN должен состоять из 6 цифр');setStaffSaving(true);const r=await apiStaff('PATCH',{id,full_name:staffDraft.full_name.trim(),role:staffDraft.role,active:staff.find(x=>x.id===id)?.active,pin:staffDraft.pin||undefined});setStaffSaving(false);if(!r.ok)return toast(r.message||'Не удалось сохранить');setEditingStaff(null);toast('Данные сотрудника обновлены');await onLog('Изменил сотрудника','staff',id,{name:staffDraft.full_name.trim(),role:staffDraft.role,pin_changed:!!staffDraft.pin});await loadStaff()}
