...
type CheckItem = { id?:number; order_id?:number; item_name:string; qty:number; unit_price:number; details:Record<string,string>; paid?:boolean; paid_amount?:number; paid_method?:string|null; deposit_amount?:number };
...
export default function Page() {
  ...
  async function loadOrders(show=true){
    ...
    try{
      ...
      if(ids.length){
        ...
        if(itemIds.length){const {data:pp,error:pe}=await sb.from('order_item_payments').select('order_item_id,amount,payment_method,deposit_amount').in('order_item_id',itemIds);if(pe)throw pe;for(const x of pp||[])paidMap.set(Number(x.order_item_id),x);}
        for(const x of oi||[]){
  ...
  async function openCheck(t:RestaurantTable){
    ...
    if(itemIds.length){const {data:pp,error:pe}=await sb.from('order_item_payments').select('order_item_id,amount,payment_method,deposit_amount').in('order_item_id',itemIds);if(pe)return setToast(pe.message);paidMap=new Map((pp||[]).map((x:any)=>[Number(x.order_item_id),x]));}
    const items=(oi||[]).map((x:any)=>{const p=paidMap.get(Number(x.id));return {id:Number(x.id),order_id:Number(x.order_id),item_name:x.item_name,qty:Number(x.qty),unit_price:Number(x.unit_price),details:x.details||{},paid:!!p,paid_amount:p?Number(p.amount):0,paid_method:p?.payment_method||null,deposit_amount:p?Number(p.deposit_amount||0):0};});
  ...
  async function closeTable(){
    ...
    const itemIds=(oi||[]).map((x:any)=>Number(x.id));
    let paidMap=new Map<number,any>();
    if(itemIds.length){const {data:pp,error:pe}=await sb.from('order_item_payments').select('order_item_id,amount,payment_method,deposit_amount').in('order_item_id',itemIds);if(pe){setCloseSaving(false);return setToast(pe.message)}paidMap=new Map((pp||[]).map((x:any)=>[Number(x.order_item_id),x]));}
    ...
    const {data:depositRows,error:dre}=await sb.from('cash_register_receipts').select('total').eq('table_id',check.table_id).eq('shift_id',currentShift.id).eq('receipt_type','deposit');
    if(dre){setCloseSaving(false);return setToast(dre.message)}
    const totalDepositsForTable=(depositRows||[]).reduce((sum:any,r:any)=>sum+Number(r.total||0),0);
    const separateDepositUsed=(oi||[]).reduce((sum:any,x:any)=>sum+Number(paidMap.get(Number(x.id))?.deposit_amount||0),0);
    // Часть депозита могла быть уже распределена на отдельно оплаченную позицию.
    // Оставшаяся часть первоначального депозита относится к остальным позициям стола.
    const depositAvailableForUnpaid=Math.max(0,totalDepositsForTable-separateDepositUsed);
    const depositUsed=Math.min(depositAvailableForUnpaid,discountedRounded);
    ...
    const now=new Date().toISOString();
    const depositUsedAtClose=Math.min(deposit,Math.max(0,discountedRounded-Math.min(separateDepositUsed,discountedRounded)));
    const paymentDetails={deposit,deposit_used:depositUsed,deposit_used_on_separate_items:separateDepositUsed,deposit_used_at_close:depositUsedAtClose,deposit_remaining:Math.max(0,deposit-depositUsedAtClose),separately_paid:Number(check.separatePaid||0),unpaid_before_close:unpaidSubtotal,discount_percent:pct,final_additional_payment:finalTotal,items:(oi||[]).map((x:any)=>{const p=paidMap.get(Number(x.id));return {order_item_id:Number(x.id),item_name:x.item_name,qty:Number(x.qty),amount:Number(x.qty||0)*Number(x.unit_price||0),paid_separately:!!p,payment_method:p?.payment_method||null,deposit_amount:Number(p?.deposit_amount||0),external_payment_amount:Number(p?.amount||0)}})};
    ...
    const {error:te}=await sb.from('restaurant_tables').update({status:'free',guests:0,active_order_id:null,deposit_total:0,deposit_cash:0,deposit_card:0,deposit_transfer:0}).eq('id',check.table_id);
    setCloseSaving(false);if(te)return setToast(te.message);
    await logAction('Закрыл стол и принял оплату','table',check.table_id,{table:check.table_name,orders_total:check.total,deposit,deposit_used:depositUsed,deposit_used_on_separate_items:separateDepositUsed,deposit_used_at_close:depositUsedAtClose,deposit_remaining:Math.max(0,deposit-depositUsedAtClose),separately_paid:Number(check.separatePaid||0),additional_payment:finalTotal,discount_percent:pct,payment_method:finalTotal>0?paymentMethod:null,receipt_id:receipt.id,guests:check.guests});
  ...
  async function saveItemPayment(amount:number){
    ...
    const currentDeposit=Number(table.deposit_total||0);
    const depositUsed=Math.min(currentDeposit,expected);
    const externalAmount=Math.max(0,expected-depositUsed);
    const pm=paymentNumbersForTotal(externalAmount);
    if(externalAmount>0&&paymentMethod==='cash'&&!pm.valid)return setToast(`Недостаточно наличных. Нужно ещё ${money(externalAmount-(Number(cashReceived)||0))}`);
    if(externalAmount>0&&paymentMethod==='mixed'&&!pm.valid)return setToast(`Наличные + карта должны быть ровно ${money(externalAmount)}`);
    setCloseSaving(true);const now=new Date().toISOString();
    let receiptId:number|null=null;
    if(externalAmount>0){
      const {data:receipt,error:re}=await sb.from('cash_register_receipts').insert({table_id:table.id,shift_id:currentShift.id,order_ids:[orderId],guests:table.guests,total:externalAmount,discount_percent:0,payment_method:paymentMethod,cash_amount:pm.cashAmount,card_amount:pm.cardAmount,transfer_amount:pm.transferAmount||0,cash_received:pm.cashReceived,change_amount:pm.change,staff_id:profile!.id,receipt_type:'item',payment_details:{order_item_id:item.id,item_name:item.item_name,position_total:expected,deposit_used:depositUsed,external_payment:externalAmount},closed_at:now}).select('id').single();
      if(re||!receipt){setCloseSaving(false);return setToast(re?.message||'Не удалось сохранить оплату')}
      receiptId=Number(receipt.id);
    }
    const {error:pe}=await sb.from('order_item_payments').insert({order_item_id:item.id,order_id:orderId,table_id:table.id,shift_id:currentShift.id,staff_id:profile!.id,amount:expected,payment_method:externalAmount>0?paymentMethod:'deposit',cash_amount:pm.cashAmount,card_amount:pm.cardAmount,transfer_amount:pm.transferAmount||0,deposit_amount:depositUsed,cash_received:pm.cashReceived,change_amount:pm.change,receipt_id:receiptId});
    if(pe){if(receiptId)await sb.from('cash_register_receipts').delete().eq('id',receiptId);setCloseSaving(false);return setToast(pe.message)}
    if(depositUsed>0){
      const {data:old,error:oe}=await sb.from('restaurant_tables').select('deposit_total,deposit_cash,deposit_card,deposit_transfer').eq('id',table.id).single();
      if(oe){await sb.from('order_item_payments').delete().eq('order_item_id',item.id);if(receiptId)await sb.from('cash_register_receipts').delete().eq('id',receiptId);setCloseSaving(false);return setToast(oe.message)}
      const oldTotal=Number(old?.deposit_total||0), oldCash=Number(old?.deposit_cash||0), oldCard=Number(old?.deposit_card||0), oldTransfer=Number(old?.deposit_transfer||0);
      const ratio=oldTotal>0?Math.min(1,depositUsed/oldTotal):0;
      const nextTotal=Math.max(0,oldTotal-depositUsed);
      const nextCash=Math.max(0,oldCash-(oldCash*ratio));
      const nextCard=Math.max(0,oldCard-(oldCard*ratio));
      const nextTransfer=Math.max(0,oldTransfer-(oldTransfer*ratio));
      const {data:updated,error:te}=await sb.from('restaurant_tables').update({deposit_total:nextTotal,deposit_cash:nextCash,deposit_card:nextCard,deposit_transfer:nextTransfer}).eq('id',table.id).select('id,name,is_vip,is_bar,status,guests,active_order_id,active,deposit_total,deposit_cash,deposit_card,deposit_transfer').single();
      if(te){await sb.from('order_item_payments').delete().eq('order_item_id',item.id);if(receiptId)await sb.from('cash_register_receipts').delete().eq('id',receiptId);setCloseSaving(false);return setToast(te.message)}
      setTables(v=>v.map(x=>x.id===table.id?updated as RestaurantTable:x));
      setTableDetail(v=>v&&v.id===table.id?updated as RestaurantTable:v);
    }
    setCloseSaving(false);setItemPayment(null);setPaymentMethod('card');setCashReceived('');setMixedCash('');setMixedCard('');setToast(`${item.item_name} · оплачено отдельно · ${money(expected)}${depositUsed>0?` (депозит ${money(depositUsed)} · доплата ${money(externalAmount)})`:''}`);await loadOrders(false);await loadTables();
  }
...
function ItemPaymentModal({table,item,paymentMethod,setPaymentMethod,cashReceived,setCashReceived,mixedCash,setMixedCash,mixedCard,setMixedCard,onClose,onConfirm,saving}:{table:RestaurantTable;item:OrderItem;paymentMethod:'cash'|'card'|'mixed'|'transfer';setPaymentMethod:(x:'cash'|'card'|'mixed'|'transfer')=>void;cashReceived:string;setCashReceived:(x:string)=>void;mixedCash:string;setMixedCash:(x:string)=>void;mixedCard:string;setMixedCard:(x:string)=>void;onClose:()=>void;onConfirm:()=>void;saving:boolean}){const total=Number(item.unit_price)*Number(item.qty);const deposit=Math.min(Number(table.deposit_total||0),total);const external=Math.max(0,total-deposit);const received=Number(cashReceived)||0,mc=Number(mixedCash)||0,mcard=Number(mixedCard)||0,mixedValid=Math.abs(mc+mcard-external)<0.01;return <div className="modal-backdrop"><div className="check-modal"><div className="modal-head"><div><div className="brand">ОТДЕЛЬНАЯ ОПЛАТА</div><h2>{item.item_name}</h2><span className="muted">{item.qty} × {money(Number(item.unit_price))} · {table.name}</span></div><button className="icon-btn" onClick={onClose}><X/></button></div><div className="payment-result"><span>Стоимость позиции</span><b>{money(total)}</b></div>{deposit>0&&<div className="payment-result"><span>Из депозита</span><b>− {money(deposit)}</b></div>}<div className="check-total"><span>К ОПЛАТЕ ДЕНЬГАМИ</span><strong>{money(external)}</strong></div>{external>0&&<><div className="payment-title">Способ оплаты</div><div className="payment-row"><button className={paymentMethod==='cash'?'selected':''} onClick={()=>setPaymentMethod('cash')}><Banknote size={18}/> Наличные</button><button className={paymentMethod==='card'?'selected':''} onClick={()=>setPaymentMethod('card')}><CreditCard size={18}/> Карта</button><button className={paymentMethod==='mixed'?'selected':''} onClick={()=>setPaymentMethod('mixed')}><ReceiptText size={18}/> Смешанная</button><button className={paymentMethod==='transfer'?'selected':''} onClick={()=>setPaymentMethod('transfer')}><Send size={18}/> Перевод</button></div></>}{external>0&&paymentMethod==='cash'&&<div className="payment-fields"><label>Дали наличными<input type="number" min="0" step="1" value={cashReceived} onChange={e=>setCashReceived(e.target.value)} placeholder={String(external)}/></label><div className={received>=external?'payment-result':'payment-result warning'}><span>{received>=external?'Сдача':'Не хватает'}</span><b>{received>=external?money(Math.max(0,received-external)):money(external-received)}</b></div></div>}{external>0&&paymentMethod==='mixed'&&<div className="payment-fields"><div className="mixed-grid"><label>Наличными<input type="number" min="0" step="1" value={mixedCash} onChange={e=>setMixedCash(e.target.value)}/></label><label>Картой<input type="number" min="0" step="1" value={mixedCard} onChange={e=>setMixedCard(e.target.value)}/></label></div><div className={mixedValid?'payment-result':'payment-result warning'}><span>{mixedValid?'Оплата совпадает':'Нужно распределить'}</span><b>{mixedValid?money(external):money(Math.abs(external-(mc+mcard)))}</b></div></div>}{external>0&&paymentMethod==='card'&&<div className="payment-result"><span>К оплате картой</span><b>{money(external)}</b></div>}{external>0&&paymentMethod==='transfer'&&<div className="payment-result"><span>К оплате переводом</span><b>{money(external)}</b></div>}<button className="orange-btn" onClick={onConfirm} disabled={saving||(external>0&&paymentMethod==='cash'&&received<external)||(external>0&&paymentMethod==='mixed'&&!mixedValid)}>{saving?<Loader2 className="spin"/>:<CheckCircle2 size={18}/>} {saving?'СОХРАНЯЕМ...':external>0?'ОПЛАТИТЬ ПОЗИЦИЮ':'ОПЛАТИТЬ ПОЗИЦИЮ ДЕПОЗИТОМ'}</button></div></div>}
  ...
  async function loadOverviewAnalytics(){
    ...
    if(openIds.length){
      ...
      if(orderIds.length){
        ...
        let depositPaidMap=new Map<number,number>();
        if(itemIds.length){
          const {data:payments}=await sb.from('order_item_payments').select('order_item_id,amount,deposit_amount').in('order_item_id',itemIds);
          paidMap=new Map((payments||[]).map((x:any)=>[Number(x.order_item_id),Number(x.amount||0)]));
          depositPaidMap=new Map((payments||[]).map((x:any)=>[Number(x.order_item_id),Number(x.deposit_amount||0)]));
        }
        ...
        const depositRowsByTable=new Map<number,number>();
        const {data:depositReceipts}=await sb.from('cash_register_receipts').select('table_id,total').eq('shift_id',currentShift.id).eq('receipt_type','deposit').in('table_id',openIds);
        for(const r of depositReceipts||[])depositRowsByTable.set(Number(r.table_id),(depositRowsByTable.get(Number(r.table_id))||0)+Number(r.total||0));
        const separateDepositByTable=new Map<number,number>();
        for(const x of openItems||[]){
          const order=(openOrders||[]).find((o:any)=>Number(o.id)===Number(x.order_id));
          if(!order)continue;
          separateDepositByTable.set(Number(order.table_id),(separateDepositByTable.get(Number(order.table_id))||0)+(depositPaidMap.get(Number(x.id))||0));
        }
        for(const t of openTables||[]){
          const grossRemaining=tableTotals.get(Number(t.id))||0;
          const depositsTotal=depositRowsByTable.get(Number(t.id))||0;
          const depositUsedOnSeparate=separateDepositByTable.get(Number(t.id))||0;
          const depositCoverageForRemaining=Math.max(0,depositsTotal-depositUsedOnSeparate);
          openBalance+=Math.max(0,grossRemaining-depositCoverageForRemaining);
        }
      } else {
