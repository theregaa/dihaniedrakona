...
type Analytics = { total:number; cash:number; card:number; transfer:number; checks:number; avg:number; top:{name:string;qty:number}[] };
...
export default function Page() {
  ...
  async function loadRoundingSetting(){const {data,error}=await sb.from('app_settings').select('value').eq('key','rounding_step').maybeSingle();if(!error&&data){const n=Number(data.value);if(Number.isFinite(n)&&[0,50,100,150,500,1000].includes(n))setRoundingStep(n)}}
  ...
  async function loadAnalytics(from?:string,to?:string){
    ...
    const {data:r}=await sb.from('cash_register_receipts').select('total,cash_amount,card_amount,transfer_amount,closed_at').gte('closed_at',startIso).lt('closed_at',endIso).order('closed_at',{ascending:false});
    const rows=(r||[]) as any[];
    const total=rows.reduce((a,x)=>a+Number(x.total||0),0),cash=rows.reduce((a,x)=>a+Number(x.cash_amount||0),0),card=rows.reduce((a,x)=>a+Number(x.card_amount||0),0),transfer=rows.reduce((a,x)=>a+Number(x.transfer_amount||0),0);
    ...
    setAnalytics({total,cash,card,transfer,checks:rows.length,avg:rows.length?total/rows.length:0,top});
  }
...
function AdminStats({analytics,onReload}:{analytics:Analytics,onReload:(from:string,to:string)=>Promise<void>}){
  ...
  return <div className="stats-panel"><div className="period-picker"><div><small>С</small><input type="date" value={from} onChange={e=>setFrom(e.target.value)}/></div><div><small>По</small><input type="date" value={to} onChange={e=>setTo(e.target.value)}/></div><button className="mini-btn" onClick={apply} disabled={loading}>{loading?<Loader2 className="spin"/>:<CalendarDays size={15}/>} Применить</button></div><div className="stats-period">Период: {new Date(`${from}T00:00:00`).toLocaleDateString('ru-RU')} — {new Date(`${to}T00:00:00`).toLocaleDateString('ru-RU')}</div><div className="stats-grid"><div><small>Выручка</small><b>{money(analytics.total)}</b></div><div><small>Чеков</small><b>{analytics.checks}</b></div><div><small>Средний чек</small><b>{money(analytics.avg)}</b></div><div><small>Наличные</small><b>{money(analytics.cash)}</b></div><div><small>Карта</small><b>{money(analytics.card)}</b></div><div><small>Переводы</small><b>{money(analytics.transfer)}</b></div></div><div className="admin-form"><b>Самые продаваемые позиции</b>{analytics.top.map((x,i)=><div className="stat-line" key={x.name}><span>{i+1}. {x.name}</span><strong>{x.qty} шт.</strong></div>)}{!analytics.top.length&&<p className="muted">Продаж за выбранный период нет.</p>}</div></div>}
