...
-- Active staff can delete an order only when it has no positions.
...
-- 3) Отдельная верхнеуровневая категория «Свой алкоголь».
DO $$
DECLARE
  nested_id bigint;
  top_id bigint;
BEGIN
  select id into nested_id from public.menu_categories where name='Свой алкоголь' and parent_id=(select id from public.menu_categories where name='Алкоголь' and parent_id is null limit 1) limit 1;
  ...
  if top_id is null and nested_id is not null then
    ...
    insert into public.menu_categories(name,icon,sort_order,active,parent_id) select 'Свой алкоголь','🍾',coalesce(max(sort_order),0)+1,true,null from public.menu_categories;
...
insert into public.menu_items(name,price,large_price,description,category_id,active)
select v.name,v.price::numeric,null::numeric,null,(select id from public.menu_categories where name='Свой алкоголь' and parent_id is null limit 1),true
from (values
  ('Свое пиво 0.5л',50),
...
where not exists (select 1 from public.menu_items i where i.name=v.name and i.category_id=(select id from public.menu_categories where name='Свой алкоголь' and parent_id is null limit 1));

-- ============================================================
-- 16. ДЕПОЗИТЫ И ОТДЕЛЬНАЯ ОПЛАТА ПОЗИЦИЙ (v128)
-- ============================================================
alter table public.restaurant_tables
...
create policy "staff can read item payments" on public.order_item_payments for select to authenticated using (exists (select 1 from public.staff_profiles s where s.id=auth.uid() and s.active=true));
create policy "waiter or admin can create item payments" on public.order_item_payments for insert to authenticated with check (exists (select 1 from public.staff_profiles s where s.id=auth.uid() and s.active=true and s.role in ('waiter','admin')));
